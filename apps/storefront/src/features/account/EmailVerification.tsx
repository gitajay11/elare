...
import { maskEmail, useEmailOtp, type OtpStatus } from './useEmailOtp';

const SUCCESS = '#5F9F72';
const ERROR = '#C85C6B';

/**
 * The "Verify your email" step of sign-up (and of sign-in for an account that
 * never finished it). Presentation only — the logic is in useEmailOtp and the
 * animation in <OtpCode>.
 */
export function EmailVerification({ email, onVerified, onBack }: { email: string; onVerified: () => Promise<void>; onBack: () => void }) {
  ...
  const proceed = async () => {
    setContinuing(true);
    try { await onVerified(); } finally { setContinuing(false); }
  };
  ...
  return (
    <div className="text-center">
      ...
      <p className="mx-auto mt-3 max-w-[20rem] text-[14.5px] leading-relaxed text-ink-soft">
        We’ve sent a 4-digit code to <span className="font-semibold text-ink">{maskEmail(email)}</span>. Enter it below to continue.
      </p>
      ...
      <div id={statusId} className="flex min-h-[3.25rem] flex-col items-center justify-center px-2" aria-live="polite">
        <AnimatePresence mode="wait" initial={false}>
          <StatusLine key={status + (otp.message?.text ?? '')} status={status} message={otp.message?.text} tone={otp.message?.tone} />
        </AnimatePresence>
      </div>

      {status === 'success' ? (
        <PillButton onClick={proceed} loading={continuing} loadingLabel="Signing you in…">Continue</PillButton>
      ...
      {status !== 'success' && (
        <div className="mt-5 text-[13px] text-ink-soft">
          ...
          <p className="mt-3 text-[12px] text-mist">Check your spam folder too. <button type="button" onClick={onBack} className="font-semibold text-ink-soft underline-offset-4 hover:underline">Use a different email</button></p>
        </div>
      )}
    </div>
  );
...
function StatusLine({ status, message, tone }: { status: OtpStatus; message?: string; tone?: 'info' | 'error' | 'success' }) {

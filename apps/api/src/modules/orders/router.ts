...
export const ordersRouter = new Hono<Env>()
  ...
  .post('/:id/cancel', async (c) => {
    ...
    return c.json(await call(userOf(c), 'cancel_my_order', { p_order_id: parse(uuid, c.req.param('id')), p_reason: reason ?? null }));
  })
  .post('/:id/refund-request', async (c) => {
    const { reason } = await body(c, refundSchema);
    return c.json(await call(userOf(c), 'request_refund', { p_order_id: parse(uuid, c.req.param('id')), p_reason: reason }));
  });
...
export const adminOrdersRouter = new Hono<Env>()
  ...
  .patch('/:id', async (c) => {
    ...
    return c.json(await call(c.get('caller'), 'admin_update_order', {
      p_order_id: parse(uuid, c.req.param('id')),

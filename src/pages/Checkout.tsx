...
import { useCart, toInputs } from '@/store/cart';
...
export default function Checkout() {
  ...
  return <CheckoutFlow userId={user.id} userEmail={user.email ?? ''} userName={profile?.full_name ?? ''} userPhone={profile?.phone ?? ''} />;
}

function CheckoutFlow({ userId, userEmail, userName, userPhone }: { userId: string; userEmail: string; userName: string; userPhone: string }) {
  ...
  const continueFromAddress = async () => {
    ...
    if (selectedAddress === 'new' && saveAddress) {
      try {
        const saved = await api.saveAddress({ user_id: userId, full_name: form.full_name, phone: form.phone, line1: form.line1, line2: form.line2 || null, city: form.city, state: form.state, postal_code: form.postal_code, country: 'IN', is_default: !(addresses?.length) });

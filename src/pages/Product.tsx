...
import { Badge, Breadcrumb, Price, QuantityStepper, Rating, SectionHeading, Stars, Swatch } from '@/components/ui/Primitives';
import { Accordion } from '@/components/ui/Accordion';
import { Input, Textarea } from '@/components/ui/Field';
import { PageLoader } from '@/components/ui/Spinner';
import { ProductImage } from '@/components/product/ProductImage';
import { WishlistButton } from '@/components/product/WishlistButton';
import { OptionSelector, ShadeSelector, useVariantSelection } from '@/components/product/VariantPicker';
...
function Reviews({ detail }: { detail: ProductDetail }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      const urls = await Promise.all(files.slice(0, 3).map((f) => api.uploadReviewImage(user.id, f)));

...
import { Input, Select, Toggle, Checkbox } from '@/components/ui/Field';
        ...
        {editing && (
          <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              {editing.scope === 'products' && <select multiple value={editing.product_ids ?? []} onChange={(e) => setEditing({ ...editing, product_ids: Array.from(e.target.selectedOptions).map((o) => o.value) })} className="h-32 w-full rounded-xl border border-line px-2 text-sm">{products.map(([id, name]) => <option key={id} value={id}>{name}</option>)}</select>}
            </div>

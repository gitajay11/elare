// Applies packages/db/migrations/*.sql to the Neon branch in DATABASE_URL, in order,
// each file in its own transaction, recording what ran in schema_migrations.
//
//   pnpm db:migrate            # uses DATABASE_URL from the environment or .env.local / .env
//   pnpm db:migrate -- --dry   # list pending migrations only
//
...
// Schema migrations first, then the seed (idempotent, tracked like a migration).
const files = ['migrations', 'seed'].flatMap((d) => readdirSync(join(root, d)).filter((f) => f.endsWith('.sql')).sort().map((f) => ({ name: f, path: join(root, d, f) })));
const pending = files.filter((f) => !applied.has(f.name));
if (!pending.length) console.log('Database is up to date.');
for (const { name: f, path } of pending) {
